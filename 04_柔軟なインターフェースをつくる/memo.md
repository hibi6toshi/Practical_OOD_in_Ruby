...
## 一番良い面（インターフェース）を表に出すコードを書く(４．４)

### 明示的なインターフェースを作る。

クラスを作るときは毎回インターフェースを宣言しましょう。「パブリック」インターフェースに含まれるメソッドは、次のようにあるべきです。
...
```
「オプション引数としてハッシュをとる」は新しい発見だった。
「デフォルト付きキーワードでいんじゃね」とも思うけど。。。
```

プライベートなインターフェースについても、同じような意図を持って設計します。

### コンテキストを最小限にする

パブリックインターフェースを構築する際は、そのパブリックインターフェースが他者に要求するコンテキストが最小限になることを目指します。「何を（what）」と「どのように（how）」の違いを念頭に置いてください。
パブリックメソッドを作る際は、メッセージの送り手が、クラスにどのようにその振る舞いを実装しているかを知ることなく、求めているものを得られるように作ります。

## デメテルの法則(4.5)

デメテルの法則（LoD:Law of Demeter）はオブジェクトを疎結合にするためのコーディング規則の集まりです。疎結合はたいてい美徳とされます。しかし、あくまでも設計の一要素にすぎませんから、他の競合する要望とのバランスを取らなければなりません。

### デメテルを定義する

デメテルはそこへメッセージを「送る」ことができるオブジェクトの集合を制限します。デメテルは 3 つ目のオブジェクトにメッセージを送る際に、異なる方の 2 つのオブジェクトを解することを禁止します。
デメテルの法則は、「直接の隣人にのみ話しかけるようにしよう」だとか、「ドットは 1 つしか使わないようにしよう」なんて言い方もされます。
仮に Trip の depart メソッドが次のコードのそれぞれを含んでいる場合を考えましょう。
...
行はそれぞれ、幾つものドット（ピリオド）を含むメッセージチェーンとなっています。これらのチェーンは、形式張らない表現では、「列車事故（train 　 wrecks）」と言われます。

メソッド名はそれぞれの列車の車両を表し、ドットが連結部を表します。
これらの車両は、デメテルの法則に違反している可能性を示しています。

### 法則を違反することによる影響

上記のメソッドチェーンの問題点。

- もし wheel が、tire もしくは rotate を変更すると、depart もかわらなければならないかもしれません。Trip は wheel と無関係であるにも関わらず、wheel の変更によって Trip への」変更が強制される恐れがあります。これは、不必要に変更コストを上げています。したがってこのコードは「合理的」ではありません。
- tire もしくは rotate の変更は depart ないの何かを壊す可能性があります。Trip は遠くにあり、明らかに関係がないので、この破壊は完全に予想外です。このコードは「見通しが良い」ものではありません。
- Trip を再利用するためには、wheel と tire を持つ bicycle を持つ customer にアクセスできるようにする必要があります。Trip はかなりのコンテキストを必要とするので、「利用性が高い」ものではありません。
- このメッセージのパターンは、他の人によって複製され、似たような問題を抱えるコードが再生産されています。残念ながら、この形式のコードは自己増殖します。「模範的」なものではありません。

### 違反を回避する

コードか「列車事故」を除去するための一般的な方法の一つに、委譲を使い、「ドット」を避ける方法があります。オブジェクト指向の用語で、メッセージを「移譲する」とは、他のオブジェクトにメッセージを渡すことです。これにはしばしばラッパーメソッドを介して行われます。

### デメテルに耳を傾ける

デメテルが何かを伝えようとしているのは確かですが、それには決して「もっと委譲を使いましょう」ということではありません。
customer.bicycle.wheel.rotate のようなメッセージチェーンが現れるとき、設計者の設計思想は、既知のオブジェクトに影響を受け過ぎています。
遠くにある振る舞いを実行するために、異なる種類のオブジェクトを横断するのは「あそこに、今ここでほしい振る舞いがある。それを『そのように手に入れたら良いかを知っている。』」と言っているのも同然です。コード自身が「何を」求めているのか（回転させること）を知るだけでなく、その求める振る舞いを得るために「どのように」いくつもの中間オブジェクトを通っていけば良いのかも知っています。
ここでは、depart メソッドが、どのように一連のオブジェクトを通過すれば車輪を回転（rotate）させられるのかを知っているために、オブジェクト全体の構造に密接に結合しています。

この結合は、あらゆる種類の問題を引き起こします。最も明らかなのは、メッセージチェーン内のどこかで起こる関係のない変更によって、Trip への変更が余儀なくされるリスクが高まったことです。しかし、ここにはより深刻な問題が潜んでいます。

depart メソッドがこのオブジェクトの連鎖を知っているとき、depart メソッドは自信をかなり特定の実装に縛り付けています。そのため、他のどんなコンテキストでも再利用できません。Customer は常に Bicycle を持つ必要があり、Bicycle は Wheel を必要とします。Wheel は rotate できるものである必要があります。

このメソッドチェーンを再考してみましょう。
depart が customer に「何を」求めているじかからを決めるとしたら、どのようなものでしょうか。メッセージに基づく視点に立てば、答えは明らかです。

```
customer.ride
```

customer の ride メソッドは、Trip から実装に」詳細を隠します。そして、コンテキストと依存関係の両方を減らすことで、設計を大幅に改善しています。FastFeet 社の方針が変わり、ハイキングの旅行も取り仕切るようになったとしましょう。そのとき、customer｡ride であれば、そこから customer.depart や customer｡go のように一般化するのは比較的容易なことです。
アプリケーションからこのメッセージチェーンの触手を解きほぐすことに比べれば、かなり簡単になります。
